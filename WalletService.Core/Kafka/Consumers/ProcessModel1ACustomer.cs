...
public class ProcessModel1AConsumer : BaseKafkaConsumer
{
    public ProcessModel1AConsumer(
    ...
    {
        var message = JsonSerializer.Deserialize<Model1AMessage>(e.Message.Value);
        try
        {
            Logger.LogInformation("[ProcessModel1AConsumer] OnMessage | Init");
            return message is not null ? Process(message) : Task.FromResult(false);
        }
        catch (Exception ex)
        {
            Logger.LogError($"[ProcessModel1AConsumer] ERROR | Exception ${ex.ToJsonString()}");
            return Task.FromResult(false);
        }
        finally
        {
            Logger.LogInformation("[ProcessModel1AConsumer] OnMessage | End");
        }
    }

    private async Task<bool> Process(Model1AMessage message)
    {
        using var scope            = ServiceScopeFactory.CreateScope();
        var       walletRepository = scope.ServiceProvider.GetService<IWalletRepository>();

        var ecoPoolsType = new List<Model1AType>();
        var levelsType   = new List<Model1ALevelsType>();

        var request = new Model1ATransactionRequest
        {
        ...
        await walletRepository!.CreateModel1ASp(request);
        Logger.LogInformation($"[ProcessModel1AConsumer] | EcoPoolProcess | Batch Completed");

        return true;
    }
    
    private void CalculateModelTwoWithOutMonth(Model1AMessage message, List<Model1ALevelsType> levelsType, List<Model1AType> ecoPoolsType)
    {
        foreach (var pool in message.ItemWithOutMonth)
        {
            ...
            ecoPoolsType.Add(new Model1AType
            {
